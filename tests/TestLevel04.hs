...
import           Test.Tasty.Wai                 ( assertBody
                                                , assertStatus'
                                                , get
                                                , post
                                                , testWai
...
main = defaultMain $ testGroup
    ...
    , testWai Core.app "View Topic" $ do
        response <- get "puppies/view"
        assertStatus' HTTP.status200 response
        assertBody "topic: fudge" response
    , testWai Core.app "Add Topic" $ do
        response <- post "puppies/add" "what a great puppy!"
        assertStatus' HTTP.status200 response
        assertBody "topic/comment: fudge / what a great fudge!" response
    , testWai Core.app "Empty Input" $ do
        response <- post "puppies/add" ""
        assertStatus' HTTP.status400 response
        assertBody "Empty Comment Text" response
    , testWai Core.app "Undefined Request" $ do
        response <- post "puppies/remove" ""
